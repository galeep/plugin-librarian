...
def cmd_impact(args):
    ...
    if not baseline_files:
        print(f"No baseline files found for '{baseline_spec}'.")
        print("Install some plugins first, or specify a marketplace as baseline.")
        sys.exit(1)
...
def scan_capabilities(marketplace_path: Path, skill_only: bool = False) -> list[Capability]:
    ...
    for md_file in marketplace_path.rglob("*.md"):
        if "backup" in str(md_file).lower():
            continue
        if skill_only and md_file.name != "SKILL.md":
            continue
...
def build_capability_index(scan_dir: Path = None, skill_only: bool = False) -> list[Capability]:
    """Build capability index from all marketplaces.

    Args:
        scan_dir: Directory to scan. Defaults to MARKETPLACES_DIR.
        skill_only: If True, only index SKILL.md files.

    Returns:
        List of all capabilities found across marketplaces
    """
    target_dir = scan_dir or MARKETPLACES_DIR
    all_caps = []
    for mp in sorted(target_dir.iterdir()):
        if not mp.is_dir() or mp.name.startswith("."):
            continue
        caps = scan_capabilities(mp, skill_only=skill_only)
...
def find_skill_file(spec: str) -> tuple[Path, str, str] | None:
    ...
    """
    # Try as direct path first, but only if within the marketplaces tree
    direct_path = Path(spec)
    if direct_path.exists() and direct_path.suffix == ".md":
        try:
            direct_path.resolve().relative_to(MARKETPLACES_DIR.resolve())
        except ValueError:
            pass  # Not within marketplaces dir, fall through to structured lookup
        else:
    ...
    if len(parts) == 1:
        ...
        mp_path_resolved = mp_path.resolve()
        for candidate in candidates:
            if candidate.exists():
                try:
                    candidate.resolve().relative_to(mp_path_resolved)
                except ValueError:
                    continue  # Skip paths that escape the marketplace tree
...
def parse_skill_file(file_path: Path, marketplace: str, plugin: str) -> SkillInfo:
    ...
    if file_path.stat().st_size > 10 * 1024 * 1024:
        raise OSError(f"File too large: {file_path}")
...
def cmd_scan(args):
    ...
    if args.index_only:
        scan_dir = Path(args.dir) if args.dir else MARKETPLACES_DIR
        if not scan_dir.exists():
            print(f"Error: Directory not found: {scan_dir}")
            sys.exit(1)
        print(f"Rebuilding capability index from {scan_dir}...")
        capabilities = build_capability_index(scan_dir=scan_dir, skill_only=args.skill_only)
        save_capability_index(capabilities)
        skills = sum(1 for c in capabilities if c.kind == "skill")
        agents = sum(1 for c in capabilities if c.kind == "agent")
        print(f"Indexed {len(capabilities)} capabilities ({skills} skills, {agents} agents)")
        print(f"Saved to: {CAPABILITY_INDEX}")
        return
    ...
    if skill_only:
        print(f"Mode: SKILL.md files only")
    print(f"Scanning marketplaces in {scan_dir}...")
    ...
    for mp in sorted(scan_dir.iterdir()):
        ...
        for md_file in mp.rglob("*.md"):
            ...
            try:
                # Skip oversized files to prevent OOM from malicious content
                if md_file.stat().st_size > 10 * 1024 * 1024:
                    print(f"  Warning: skipping oversized file {md_file}", file=sys.stderr)
                    continue
                content = md_file.read_text(encoding="utf-8", errors="replace")
                if len(content) < 100:
                    continue
...
def main():
    ...
    # scan
    scan_p = subparsers.add_parser("scan", help="Scan marketplaces and build similarity index")
    scan_p.add_argument("--dir", "-d", default=None,
                        help="Directory to scan (default: ~/.claude/plugins/marketplaces)")
    scan_p.add_argument("--skill-only", action="store_true",
                        help="Only scan SKILL.md files (ignore READMEs, reference docs, etc.)")
    scan_p.add_argument("--exclude", nargs="*", default=[],
                        help="Subdirectory names to exclude (e.g. --exclude curated .git)")
    scan_p.add_argument("--threshold", type=float, default=None,
                        help="Similarity threshold 0.0-1.0 (default: 0.7)")
    scan_p.add_argument("--index-only", action="store_true",
                        help="Only rebuild capability index (skip similarity scan)")
