...
def checkout_skill(
    ...
    try:
        ...
        if skill_path.is_file():
            ...
            # Reject symlinks that point outside the source tree
            if skill_path.is_symlink():
                real_source = skill_path.resolve()
                try:
                    real_source.relative_to(skill_path.parent.resolve())
                except ValueError:
                    return CheckoutResult(
                        success=False,
                        target_path=destination,
                        files_copied=[],
                        metadata={},
                        message=f"Refusing to checkout symlink that points outside source directory"
                    )
            ...
            if skill_path.suffix == ".md" and skill_path.stat().st_size <= 10 * 1024 * 1024:
            ...
            for item in skill_path.rglob("*"):
                if not item.is_file():
                    continue
                # Reject symlinks that point outside the skill directory tree
                if item.is_symlink():
                    try:
                        item.resolve().relative_to(skill_path_resolved)
                    except ValueError:
                        continue  # Skip symlinks escaping the skill tree
            ...
            if (skill_md.exists() and not skill_md.is_symlink()
                    and skill_md.stat().st_size <= 10 * 1024 * 1024):
