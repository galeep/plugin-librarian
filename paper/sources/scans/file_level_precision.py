...
It supports the file-level precision figures of Section 4.4 (IOC Validation),
which Section 3.4 promises: 305 of 307 files (0.993) at >= 20, and at >= 10 a
membership-level 557 of 682 memberships over 643 distinct files, 0.817. The
study rows of the output are those figures with their per-cluster breakdown.
The cross-check section of the output is the computation behind the Table 4
caption, which limits what the released ground truth reproduces to the >= 20
and >= 10 rows.
...
def cross_check(measured, context, marker_reproduces, columns):
    ...
    if disagree:
        lines += [
            ...
            " overshoots it by one. The Table 4 caption limits what the released ground"
            " truth reproduces to the >= 20 and >= 10 rows. The >= 15 and >= 5 rows rest on"
            " per-cluster triage decisions that the released ground truth does not carry,"
            " so this script does not reproduce them. Differences of one cluster are what"
            " hand triage under Section 3.4's third clause would produce.",
            "",
...
def preamble(antiy, study, indicators, marker, clusters, is_study, content_all):
    ...
    return [
        ...
        "",
        "This file reports, for the clusters of the scan with >= 20, >= 10 and >= 5 files,"
        " the share of files that are malicious under three ground truths, the"
        " cluster-level true positives each ground truth yields, and how many of those"
        " clusters are pure versus mixed. It supports the file-level precision figures of"
        " Section 4.4 (IOC Validation), which Section 3.4 promises: 305 of 307 files"
        " (0.993) at >= 20, and at >= 10 a membership-level 557 of 682 memberships over 643"
        " distinct files, 0.817. The study rows below are those figures with their"
        " per-cluster breakdown. The cross-check section is the computation behind the"
        " Table 4 caption, which limits what the released ground truth reproduces to the >="
        " 20 and >= 10 rows. Section 3.4 (Ground Truth and Validation) defines the two"
        " levels: cluster-level precision is the fraction of clusters holding confirmed"
        " malicious content, and file-level precision the fraction of clustered files"
        " confirmed malicious.",
        "",
        "## Definitions",
        "",
