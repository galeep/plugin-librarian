...
which sets TF-IDF beside MinHash on cluster count, coverage, recall, precision and
wall-clock runtime, and the TF-IDF bars of Figure 3. The script vectorises
...
# The Figure 3 caption and Section 8 (Conclusion) give the paper's bench as an
# 8-core Intel i9-9880H with 64 GB. Recorded here so a rerun states whether it is
# the same machine instead of leaving the runtime comparison to a reader's
# assumption.
PAPER_MACHINE = ('Intel i9-9880H, 8 cores, 64 GB; the Figure 3 caption records '
                 'wall-clock runtime on an 8-core i9-9880H')
...
# The TF-IDF cluster and coverage figures of Section 5.2
...
def mode_note(decode, n_texts, n_index):
    ...
    if decode == "strict":
        head = (f"**This is the `--decode strict` run, and it reproduces the paper's "
                ...
                f"population behind the Section 5.2 figures, so this run vectorizes "
...
def build_report(scan, texts, metadata, skipped, dropped, clusters, timings,
    ...
    lines = [
        ...
        "Baselines), which sets TF-IDF beside MinHash on cluster count, coverage, "
        "recall, precision at the >= 20 and >= 10 cluster sizes, and wall-clock "
        "runtime. It also supports the TF-IDF bars of Figure 3.", "",
    ...
    lines += [
        ...
        "Paper source: the TF-IDF cluster and coverage figures of Section 5.2, and "
        "the hardcoded TF-IDF bars in "
    ...
    lines += [
        ...
        + ("The CPU is the same model the paper benched on, the 8-core Intel "
           "i9-9880H with 64 GB that the Figure 3 caption and Section 8 (Conclusion) "
           "record, but this run ran at the load average "
