...
def main() -> int:
    ...
    lines = ["# Shingle size ablation (90% Jaccard threshold, SKILL.md only)", "",
             "This file reports MinHash/LSH clustering of the SKILL.md corpus at the 90% "
             "Jaccard threshold with 2-gram, 3-gram and 4-gram word shingles, with precision "
             "and recall for each size. It reproduces Table 5 (Section 5.2, Comparison with "
             "Simple Baselines) and supports the paragraph that introduces that table, which "
             "repeats the MinHash analysis at 2-gram and 4-gram shingles beside the 3-gram "
             "default. That paragraph reports the same hightower6eu recall at all three sizes, "
             "more topical false positives at 2-grams, one campaign fragmented at 4-grams, and "
             "no size ahead of the others at the two gates the paper reports, >= 20 and >= 10: "
             "2-grams tie 3-grams at >= 20 and 4-grams lead at >= 10.", "",
