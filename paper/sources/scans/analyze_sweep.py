#!/usr/bin/env python3
"""Threshold sweep analysis: precision and recall against the IOC list.

Reads every archived `scan_*_threshold*_skillonly.json` beside this script and reports,
per threshold: total clusters, files in clusters, cluster rate, precision at cluster
size thresholds >=20, >=10 and >=5, recall against the documented hightower6eu skill
slugs, and scaffold cluster counts. Backs Table 2 (threshold sensitivity) and Figure 1
of Section 4.1.

A cluster counts as a true positive when any of its files sits under a documented
attacker account, matched on the account segment of the ClawHub archive path.

Inputs: the six archived scans beside this script, and `paper/iocs.json` for the
account list and the recall slugs. No corpus access, no network, nothing randomised.
...
# Known malicious authors from iocs.json
MALICIOUS_AUTHORS = {
...
# Koi IOC slugs for recall (hightower6eu, 341 in original report)
# We use the full slug list from iocs.json for hightower6eu
IOC_FILE = Path(__file__).parent.parent.parent / "iocs.json"


def load_ioc_slugs():
    """Load hightower6eu skill slugs for recall calculation."""
    with open(IOC_FILE) as f:
        data = json.load(f)
    for author_data in data["clawhub_authors"].values():
        pass
    # Actually it's a dict keyed by username
    authors = data["clawhub_authors"]
...
def is_malicious_cluster(cluster, file_index):
    """Check if a cluster contains files from known malicious authors."""
    for loc in cluster.get("locations", []):
        marketplace = loc.get("marketplace", "")
        # ClawHub skills: marketplace is "skills" (the openclaw/skills repo)
        # The plugin name often contains the author as a prefix
        plugin = loc.get("plugin", "")
        path = loc.get("path", "")

        # Check if any path component matches a malicious author
        parts = path.split("/")
        for part in parts:
            if part in MALICIOUS_AUTHORS:
                return True

        # Also check the file_index entry for marketplace info
...
def analyze_scan(scan_path):
    ...
    # Load IOC slugs for recall
    ioc_slugs = load_ioc_slugs()

    # Track which IOC slugs appear in any cluster
    found_ioc_slugs = set()
    for c in clusters:
        for loc in c.get("locations", []):
            ...
            if fidx is not None and fidx < len(file_index):
                ...
                path = fi.get("path", "")
                # Check if this is a hightower6eu skill
                if "hightower6eu" in path:
                    # Extract slug from path like hightower6eu/slug-name/SKILL.md
                    parts = path.split("/")
                    for i, p in enumerate(parts):
                        if p == "hightower6eu" and i + 1 < len(parts):
                            found_ioc_slugs.add(parts[i + 1])

    # Precision at different size thresholds
