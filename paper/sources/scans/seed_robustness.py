...
def write_results(scan, files, results, tallies, rows):
    ...
    lines = [
        ...
        "Rand index and adjusted Rand index for every pair of seeds. It supports two results in "
        "Section 3.3 (Similarity Analysis): the range of cluster counts across the five fixed "
        "seeds, with the default seed reproducing the archived scan exactly, and the spread in "
        "distinct clustered files across the same five. Both come from the per-seed table below. "
        "The adjusted Rand index range that section reports, 0.931 to 0.955, is computed over "
        "every file clustered under either seed with the remainder as singletons; "
        "`recompute_ari.py` reports that population as `ari_union` in "
        "`recompute-ari-results-20260907.json`. The pairwise table here scores the files "
        "clustered under both seeds, which is that file's `ari_inter` population, and the two "
        "agree on it.",
        "",
